...
def test_ai_news_run_accepts_manual_story(monkeypatch) -> None:
    from app.models import AiNewsRunResult

    async def fake_run_ai_news_pipeline(**kwargs):
        assert kwargs["manual_story"].url == "https://thefrontpage.dev/"
        assert kwargs["manual_story"].title == "The Front Page"
        assert kwargs["publish"] is False
        return AiNewsRunResult(
            run_id="test-run",
            status="dry_run",
            story_url=kwargs["manual_story"].url,
            story_title=kwargs["manual_story"].title,
            article_slug="the-front-page-hacker-news-newspaper",
            image_filename="the-front-page-hacker-news-newspaper.jpg",
            caption="Draft caption",
            dry_run=True,
        )

    monkeypatch.setattr("app.main.run_ai_news_pipeline", fake_run_ai_news_pipeline)

    response = client.post(
        "/api/ai-news/run",
        json={
            "story_url": "https://thefrontpage.dev/",
            "story_title": "The Front Page",
            "story_summary": "Hacker News as a newspaper.",
            "dry_run": True,
            "publish": False,
        },
    )
    ...
    payload = response.json()
    assert payload["status"] == "dry_run"
    assert payload["article_slug"] == "the-front-page-hacker-news-newspaper"
    assert payload["story_url"] == "https://thefrontpage.dev/"
