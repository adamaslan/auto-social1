...
def ai_news_max_candidates() -> int:
    return _env_int("AI_NEWS_MAX_CANDIDATES", default=5, minimum=1)


def ai_news_image_provider() -> str:
    provider = env_value("AI_NEWS_IMAGE_PROVIDER", default="gemini").lower()
    return provider if provider in {"gemini", "pollinations"} else "gemini"


def ai_news_dry_run_default() -> bool:
    return _env_bool("AI_NEWS_DRY_RUN_DEFAULT", default=True)
