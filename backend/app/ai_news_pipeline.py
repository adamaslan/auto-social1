...
Eight steps:
  ...
  6. write_to_ttb8        — drops files into ttb8 repo, updates route/index files
  7. compose_caption      — Gemini → ≤ 2200-char Instagram caption
  8. publish_to_instagram — calls existing /api/publish endpoint

The sibling ttb8 app currently registers articles in app/routes.ts and
app/routes/ai-articles.tsx, so this module updates those files directly.
"""
...
# House style for every auto-generated hero image. Override per-deploy with
# AI_NEWS_IMAGE_STYLE env var if a different aesthetic is ever needed.
_DEFAULT_IMAGE_STYLE = (
    "8-bit pixel art, retro game sprite style, NES palette, chunky pixels, "
    "limited palette, CRT scanline feel, square 1:1, no text, no logos"
)


def _image_style_suffix() -> str:
    return env_value("AI_NEWS_IMAGE_STYLE") or _DEFAULT_IMAGE_STYLE
...
async def run_ai_news_pipeline(
    *,
    ...
    image_provider_attempted = ai_news_image_provider()
    image_provider_used_val: str | None = None
    image_filename_val: str | None = None
    image_bytes_val: int | None = None
    image_width_val: int | None = None
    image_height_val: int | None = None
    image_aspect_ratio_val: float | None = None
    publish_log_id = ""
    media_id = ""
    ...
    try:
        if manual_story:
            story = manual_story
            record_event("ai_news_manual_story_supplied", run_id=run_id, url=story.url, title=story.title)
            artifact_store.write(run_id, "candidates.json", [story.__dict__])
        else:
            # 1. Discover
            ...
            artifact_store.write(run_id, "candidates.json", [s.__dict__ for s in stories])
            record_event(
                "ai_news_candidates_collected",
                run_id=run_id,
            ...
            # 2. Pick
            story, picker_fallback_used, picker_provider = await _pick_story_tracked(stories, run_id)

        record_event(
            "ai_news_story_picked",
            run_id=run_id,
            url=story.url,
            title=story.title,
            fallback_used=picker_fallback_used,
        )

        # 3. Generate article
        providers = provider_status()
        if not providers.get("mistral_configured") and not providers.get("gemini_configured"):
            raise RuntimeError("Article generation requires MISTRAL_API_KEY, MISTRAL_KEY, or GEMINI_API_KEY")
        ...
        record_event(
            "ai_news_image_request",
            run_id=run_id,
            provider=image_provider_attempted,
            prompt_chars=len(article.image_prompt),
            style_suffix_chars=len(_image_style_suffix()),
        )
        artifact_store.write(run_id, "image-request.json", {
            "prompt": article.image_prompt,
            "style_suffix": _image_style_suffix(),
            "provider": image_provider_attempted,
        })
        image_filename, image_provider_used_val = await generate_hero_image(article)
        image_filename_val = image_filename

        image_path = MEDIA_DIR / image_filename
        image_bytes_val = image_path.stat().st_size if image_path.exists() else None

        # 5b. Validate the image (blue-gradient guard)
        validation = validate_hero_image(image_path)
        image_width_val = validation.width
        image_height_val = validation.height
        image_aspect_ratio_val = validation.ratio
        record_event(
            "ai_news_image_received",
            run_id=run_id,
            provider_used=image_provider_used_val,
            bytes=image_bytes_val,
            width=validation.width,
            height=validation.height,
            aspect_ratio=validation.ratio,
        )
        record_event(
            "ai_news_image_validation",
            run_id=run_id,
            aspect_ok=validation.aspect_ok,
            dimension_ok=validation.dimension_ok,
            file_format=validation.format,
            is_likely_blank=validation.is_likely_blank,
        )
        if validation.is_likely_blank:
            finish_run(run_id, "failed", "Image appears blank or uniform — aborting before publish")
            return AiNewsRunResult(
                run_id=run_id,
                status="failed_blank_image",
                dry_run=dry_run,
                image_filename=image_filename,
                image_provider_used=image_provider_used_val or "",
                error="Hero image failed blank-image validation (all channels low stddev)",
            )
        ...
        duration_ms = round((_time.perf_counter() - pipeline_start) * 1000)
        record_event(
            "ai_news_run_completed",
            run_id=run_id,
            status="published" if (publish and not dry_run) else "dry_run",
            duration_ms=duration_ms,
        )
        finish_run(run_id, "completed", f"Generated article: {article.slug}")
        return AiNewsRunResult(
            run_id=run_id,
            status="published" if (publish and not dry_run) else "dry_run",
            story_url=story.url,
...
async def pick_story(stories: list[Story]) -> Story:
    story, _, _ = await _pick_story_tracked(stories, run_id="")
    return story
    ...
    try:
        ...
        for story in stories:
            if story.url == chosen_url:
                record_event(
                    "ai_news_picker_decision",
                    run_id=run_id,
                    chosen_url=chosen_url,
                    fallback_used=False,
                    candidates_count=len(stories),
                )
                return story, False, "llm"
    except Exception as exc:
        record_event("ai_news_pick_story_error", level="warning", run_id=run_id, error=str(exc))

    # Fallback: first candidate
    record_event(
        "ai_news_picker_decision",
        run_id=run_id,
        chosen_url=stories[0].url,
        fallback_used=True,
        candidates_count=len(stories),
    )
    return stories[0], True, None
...
{{
  ...
  "image_prompt": "Imagen prompt describing the SUBJECT only (e.g. 'a hacker terminal with skull and padlock icons'). Style is appended automatically — do NOT mention 8-bit, pixel, retro, abstract, or art style. No text, no logos."
}}

Inline linking rules — MANDATORY. Every article MUST contain BOTH of the following, embedded as markdown links inside paragraph sentences where they fit naturally:

1. EXTERNAL SUBJECT LINK (required, exactly 1): Link to the canonical website / GitHub repo / paper / docs page for the primary tool, library, framework, or company the article is about. Use this style: "Enter [ToolName](https://tool.example.com), a library designed to..." or "...built on [Framework](https://framework.io)...". This is in addition to the source-story link.

2. PREVIOUS-ARTICLE LINK (required, exactly 1): Link to the single most topically related article from the "Previously published articles" list above. Weave it into a sentence where the connection is genuine, not forced. Use this style: "For teams already using [DSPy for RAG efficiency](/5-ways-to-enhance-rag-efficiency-with-dspy), ToolName can..." Use the article's exact title and slug path as shown in the list.

3. SOURCE STORY LINK (required, exactly 1): Link to the source story once: [{story.title}]({story.url})

Constraints:
- Use markdown link syntax: [visible text](url) — no bare URLs in prose.
- Exactly 3 inline links total: 1 external subject + 1 previous article + 1 source story.
- Never repeat the same URL.
- The subject link and previous-article link should appear in the Introduction or an early section, not buried in the Conclusion.
...
- keywords list must have 4-6 items"""
    ...
    return Article(
        ...
        sections=sections,
        image_prompt=data.get("image_prompt", f"AI technology hero scene for {data['title']}"),
    )
...
async def write_to_ttb8(article: Article, route_tsx: str, image_filename: str) -> WriteResult:
    ...
    # Keep the optional JSON registry in sync, then update the actual ttb8 files
    # that are present in this repo today.
    _register_article(article, image_filename)
    _append_route_config(ttb8, article.slug)
    _prepend_ai_article_card(ttb8, article, image_filename)
...
def _register_article(article: Article, image_filename: str) -> None:
    """Append the new article to the optional JSON registry."""
...
def _append_route_config(ttb8: Path, slug: str) -> None:
    routes_file = ttb8 / "app" / "routes.ts"
    if not routes_file.exists():
        record_event("ai_news_routes_ts_missing", level="warning", path=str(routes_file))
        return

    text = routes_file.read_text(encoding="utf-8")
    entry = f'  route("{slug}", "./routes/{slug}.tsx"),'
    if entry in text:
        return

    marker = "] satisfies RouteConfig;"
    if marker not in text:
        raise ValueError(f"Could not find RouteConfig marker in {routes_file}")

    updated = text.replace(marker, f"{entry}\n{marker}", 1)
    routes_file.write_text(updated, encoding="utf-8")
    record_event("ai_news_route_registered", slug=slug, path=str(routes_file))


def _prepend_ai_article_card(ttb8: Path, article: Article, image_filename: str) -> None:
    ai_articles_file = ttb8 / "app" / "routes" / "ai-articles.tsx"
    if not ai_articles_file.exists():
        record_event("ai_news_articles_index_missing", level="warning", path=str(ai_articles_file))
        return

    text = ai_articles_file.read_text(encoding="utf-8")
    link = f"/{article.slug}"
    if f'link: "{link}"' in text:
        return

    marker = "  const articles = [\n"
    if marker not in text:
        raise ValueError(f"Could not find articles array in {ai_articles_file}")

    entry = (
        "    {\n"
        f"      title: {json.dumps(article.title)},\n"
        f"      description: {json.dumps(article.og_description)},\n"
        f"      link: {json.dumps(link)},\n"
        f"      image: {json.dumps('/' + image_filename)}\n"
        "    },\n"
    )
    updated = text.replace(marker, marker + entry, 1)
    ai_articles_file.write_text(updated, encoding="utf-8")
    record_event("ai_news_articles_index_updated", slug=article.slug, path=str(ai_articles_file))
